namespace :db do
   task populate: :environment do
      ...
      container = Maintopic.create!(topicname: "Topic Container Explanation",
                           description: "Topic Containers(Tcontainers for short) are the highest level of the forum and provide the building blocks neccessary to create a good forum. The thing to remember about Topic Containers is to use them sparingly to ensure that one doesn't not create too many or too little. A topic container is just a box that contains Maintopics and when one is specified only the name of the container is needed to set it up. If only a single container exist and contains more then 3 topics, say about 12 or more then that is a bad container unless of course one is trying to create a Fanfiction like forum here, but normally here that is considered a bad practice. Equally bad is when a forum contains many topic containers that only have a single maintopic per each. That is not good practice and will be frowned upon. The most optimal use of a topic container is one that contains at least 1-3 maintopics with 2-3 being prefered, this method gives the forum a clean look and feel being pleasing to the eyes of other users. To create a new Tcontainer a user would first select a forum first and then click the new Tcontainer link on that forum. Once you have done that you would then type in the container name and click the submit button, the new topic container will appear below the given forum. Try this out for yourself on the Tutorial Forum. If you do this, you will have successfully posted your first Tcontainer.",
                           user_id: 3)
